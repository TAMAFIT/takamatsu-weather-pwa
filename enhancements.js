...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('refreshButton')?.addEventListener('click', () => setTimeout(() => { fetchAmedas(); scheduleForecastPatch(); }, 1400));
  document.getElementById('amedasRefresh')?.addEventListener('click', fetchAmedas);
  document.getElementById('weeklyList')?.addEventListener('click', () => setTimeout(scheduleForecastPatch, 80));

  const tbody = document.getElementById('hourlyTableBody');
  if (tbody) {
    const observer = new MutationObserver(() => {
      if (!extState.patchingTable) setTimeout(scheduleForecastPatch, 50);
    });
...
function setupOfficialLinks() {
  const nowcast = document.getElementById('nowcastLink');
  const himawari = document.getElementById('himawariLink');
  const amedas = document.getElementById('amedasMapLink');
  if (nowcast) nowcast.href = `https://www.jma.go.jp/bosai/nowc/#lat:${EXT.lat}/lon:${EXT.lon}/zoom:11/colordepth:normal/elements:hrpns&slmcs`;
  if (himawari) himawari.href = `https://www.jma.go.jp/bosai/map.html#6/${EXT.lat}/${EXT.lon}/&elem=ir&contents=himawari`;
  if (amedas) amedas.href = `https://www.jma.go.jp/bosai/map.html#8/${EXT.lat}/${EXT.lon}/&contents=amedas`;
}

function setupAmedas() {
  fetchAmedas();
}
...
function formatJmaTime(date) {
  const parts = new Intl.DateTimeFormat('sv-SE', { timeZone: EXT.tz, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }).formatToParts(date).reduce((a, p) => (a[p.type] = p.value, a), {});
...
function findNearestStation(table, data) {
  ...
  Object.entries(table).forEach(([id, station]) => {
    const obs = data[id];
    if (!obs) return;
    const lat = toDegree(station.lat);
    const lon = toDegree(station.lon);
    if (!Number.isFinite(lat) || !Number.isFinite(lon)) return;
    const hasUseful = obs.sun1h || obs.precipitation1h || obs.temp || obs.humidity || obs.wind;
    if (!hasUseful) return;
...
function toDegree(value) {
  if (Array.isArray(value)) return Number(value[0]) + Number(value[1] || 0) / 60;
  return Number(value);
}

function valueOf(obs, key) {
  const value = obs?.[key];
...
function parseAmedasObservation(station, obs, date) {
  ...
  return {
    id: station.id,
    name: station.station.kjName || station.station.enName || station.id,
    distance: station.distance,
    time: date.toISOString(),
    sun1h: valueOf(obs, 'sun1h'),
    rain1h: valueOf(obs, 'precipitation1h'),
    temp: valueOf(obs, 'temp'),
    humidity: valueOf(obs, 'humidity'),
    windDirection,
    windSpeed
  };
}
...
function scheduleForecastPatch() {
  setTimeout(patchHourlyTableForSun, 250);
  setTimeout(renderLogSummary, 300);
}
...
function patchHourlyTableForSun() {
  ...
  tbody.innerHTML = rows.map((row) => {
    const sun = sunJudge(row);
    const adjusted = adjustedSunJudge(sun, row, extState.amedas);
    return `<tr>
      <td>${String(row.hour).padStart(2, '0')}:00</td>
      <td><span class="sun-badge ${adjusted.className}">${escapeHtml(adjusted.label)}</span><br><span class="source-note">${escapeHtml(row.weatherText || '-')}</span></td>
      <td>${formatPercent(row.cloudCover)}</td>
      <td>${formatMinutes(row.sunshineMinutes)}</td>
      <td>${formatNumber(row.uvIndex)}</td>
      <td>${formatProbability(row.precipitationProbability)} / ${Number(row.precipitation || 0).toFixed(1)}mm</td>
      <td>${escapeHtml(adjusted.reason)}</td>
    </tr>`;
  }).join('');
  extState.patchingTable = false;
}

function sunJudge(row) {
  const cloud = row.cloudCover;
  const sunshine = row.sunshineMinutes;
  const uv = row.uvIndex;
  let score = 0;
  if (row.hour < 7 || row.hour > 17) score -= 4;
  if (typeof cloud === 'number') score += cloud <= 25 ? 3 : cloud <= 50 ? 2 : cloud <= 70 ? 1 : -1;
  if (typeof sunshine === 'number') score += sunshine >= 45 ? 3 : sunshine >= 25 ? 2 : sunshine >= 10 ? 1 : -1;
  if (typeof uv === 'number') score += uv >= 6 ? 2 : uv >= 3 ? 1 : 0;
...
function adjustedSunJudge(judge, row, obs) {
  ...
  if (obs?.sun1h != null && obs.sun1h <= 0.1 && row.hour <= new Date().getHours() + 3) {
    if (judge.key === 'high') return { key: 'mid', label: '保留', className: 'sun-mid', reason: '実測日照ほぼなし' };
    return { key: 'low', label: '曇り寄り', className: 'sun-low', reason: '実測日照ほぼなし' };
  }
  return judge;
}

function setupLogs() {
  bindChoiceGroup('skyChoices', 'selectedSky');
  bindChoiceGroup('tanChoices', 'selectedTan');
...
function bindChoiceGroup(id, stateKey) {
  ...
  group.addEventListener('click', (event) => {
    ...
    extState[stateKey] = button.dataset.value;
  });
}

function saveCurrentLog() {
  if (!extState.selectedSky || !extState.selectedTan) {
    alert('実際の空と日焼け結果を両方選んでください。');
    return;
  }
  const bundle = readWeatherBundle();
  ...
  const record = {
    ...
    sky: extState.selectedSky,
    tanResult: extState.selectedTan,
    amedas: extState.amedas,
    forecast: nowHours.map((x) => ({ time: x.time, weather: x.weatherText, cloud: x.cloudCover, sunshine: x.sunshineMinutes, uv: x.uvIndex, precipProb: x.precipitationProbability, precip: x.precipitation }))
  };
  const logs = readLogs();
  logs.unshift(record);
  localStorage.setItem(EXT.logKey, JSON.stringify(logs.slice(0, 200)));
  renderLogSummary();
  alert('記録しました。');
}
...
function renderLogSummary() {
  ...
  if (!logs.length) {
    summary.textContent = 'まだ記録がありません。晴れ予報なのに曇った時ほど記録価値があります。';
    recent.innerHTML = '';
    return;
  }
...
function exportLogsCsv() {
  const logs = readLogs();
  if (!logs.length) { alert('出力できるログがありません。'); return; }
  ...
  const rows = logs.map((log) => [
    log.createdAt, log.sky, log.tanResult, log.amedas?.name || '', log.amedas?.sun1h ?? '', log.amedas?.rain1h ?? '',
    avg(log.forecast.map((x) => x.cloud)), avg(log.forecast.map((x) => x.sunshine)), avg(log.forecast.map((x) => x.uv)), avg(log.forecast.map((x) => x.precipProb))
  ]);
  const csv = [header, ...rows].map((row) => row.map(csvCell).join(',')).join('\n');
  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = 'takamatsu-weather-logs.csv'; a.click();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
}

function clearLogs() {
  if (!confirm('記録をすべて削除しますか？')) return;
  localStorage.removeItem(EXT.logKey);
  renderLogSummary();
}
